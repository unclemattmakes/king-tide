...
Debug knobs: `?wire=1` (wireframe), `?steep=N` (0..1.5 steepness override), `?reflect=0` (disable planar reflection for perf tests), `?reflectfull=1` (legacy FULL-scene mirror — since 2026-06-11 the reflection pass renders only the opt-in `WATER_REFLECTION_LAYER`: sky dome + lights + terrain/landmark meshes ≥25 m, flagged in sky.ts/scene.ts/track-loader.ts; props/bikes/FX/streamed dressing stay out by default, taking the mirror from ~98 re-encoded draw calls to ~5 with reflections kept — see the water-perf pass in status.md + `tools/water-ablation.mjs`), `?watersubs=<n>` (center-mesh density override, 64..1024 — default dropped 768²→512² in the same pass; the win saturated there), `?aa=off` (drop MSAA so the WebGPU scene-depth copy can run and shoreline foam fires from screen-space depth), `__waterSteepness(n)` and `__waterDetail(n)` console hooks, plus `waterDebug().setWaterVisible(on)` / `.setReflectionFullScene(on)` perf probes. The full FFT path and the `?water=classic` A/B fallback were removed when we settled on analytic Gerstner + SoT-style fragment shading as the only path. **Lights gotcha:** lights pass a per-camera layer test, so any light that should reach the MIRROR needs `layers.enable(WATER_REFLECTION_LAYER)` (scene.ts does this for sun + hemi) — miss it and the mirrored terrain renders unlit black and the sea reads black.
...
| Reflection resolution | `resolutionScale` arg to `reflector(...)` | 0.5 | half-res reflection target. (Reflection cost DID show up in profiling — 2026-06-11 — but the cost was the mirror's per-frame scene re-encode, not its resolution, so the fix was the layer cull below, not 0.33) |
| Reflection scene cull | `WATER_REFLECTION_LAYER` opt-in (water.ts; flagged in sky.ts, scene.ts lights, track-loader ≥25 m size gate) | on (`?reflectfull=1` = legacy) | the mirror's virtual camera renders only sky + lights + terrain/landmark silhouettes — ~5 draws instead of re-encoding the scene (~98 on sandbar, the whole dressed city on mexico-city). At the 0.55 fresnel cap + distortion the cull is visually invisible (`artifacts/water-perf/` A/B pairs) |
| Center-mesh density | `?watersubs=<n>` / `subs` in water.ts | 512 (was 768) | 1.875 m vertex spacing; the June-10 per-vertex work (trails/stamps/rings/crest signals) made density a real lever and the fps win saturates at 512² on the ablation |
