...
5. The **highest-leverage remaining work** (in rough rank order):
   ...
   - **Per-track sea-state schema**: tracks currently use
     ...
     read calmer than big-bay etc. ~45 min. Wind direction + cutoff
     sliders just landed (A5 complete) and are wired through the
     same `applySpectrumParams` path a per-track schema would use,
     so this is mostly schema work.
   - **A8 foam-feedback follow-ups**: optional wave-advection
     offset in the foam kernel (foam drifts with wave group
     velocity instead of staying anchored), and exposing the
     (jHigh, jLow, decay) thresholds as live debug-menu sliders
     for per-session tuning. Both nice-to-have, not load-bearing.
...
| --- | --- |
| (no flag — default) | **The full SoT-style FFT path** (A2 + A3 + A7 + A8). Three FFT cascades (wind sea + chop + long swell) drive vertex displacement; Jacobian alpha drives foam; choppiness peak-mask drives SSS color. Mitsuyasu cos²ˢ(α/2) directional spread + Hasselmann frequency-dependent `s` schedule. CPU buoyancy reads top-K analytic of cascade 0 (~5% gap from full-grid). FFT bake (not direct DFT) is also default; cascade resolution defaults to N=64 main + N=32 chop/swell (`?fft=hi` for N=128/64); cascades dispatch at 30Hz (`?fftskip=1` for every-frame). |
| `?water=v2` | The previous default: 6-wave Gerstner + procedural detail cascade. Pair with `?waves=gerstner` for the previous CPU buoyancy too. |
| `?water=fft` | Explicit alias for the new default (kept for old bookmarks). |
| `?waves=gerstner` | Legacy: big-wave field is the 6-wave hand-tuned Gerstner sum. CPU buoyancy samples it directly. Pairs with `?water=v2` for full legacy water. |
| `?waves=fft` | Explicit alias for the new spectrum default (kept for old bookmarks). |
| `?fftbake=ddft` | Legacy direct-DFT bake for the FFT path. Useful for A/B against the FFT bake if it regresses. Cost is O(N⁴) — only viable at small N. |
| `?water=classic` | Pre-existing legacy heightfield (vertical-only Gerstner, single-color albedo). Untouched by this migration. |
...
| A5 — Tuning + debug menu rewrite | ✅ done | **Sliders landed:** Choppiness (λ), Sea state (renderScale), Wind speed, **Wind direction** (deg CCW from world +X — rebuilds only the wind-sea cascade so the chop + long-swell cascades keep their hard-coded angles), **Wind cutoff** (Phillips smallWavelengthCutoff in m — controls chop detail vs surface smoothness). Choppiness + Sea state live-mutate the GPU kernel's uniforms via `gpuDisplacementHandle.setChoppiness` / `setRenderScale`. Wind speed / direction / cutoff each kick off a live spectrum rebuild via `applySpectrumParams` in `water.ts` — `buildPhillipsSpectrum` (CPU) + `selectTopKModes` + `gpuDisplacementHandle.uploadSpectrum(grid)`. Persisted to `hoverbike.waterDebug.v4` localStorage; older v1-v3 entries silently merge onto defaults via the per-key tolerant loader. **Visual tune landed:** initial calibration pass (windSpeed=11, amplitude=1e-6, windDir=(0.6,0.8), choppiness=0.7) — superseded by the A7 cascade tune (see below). **Stability fix:** the `waves=fft` analytic path freezes if `topK` is bumped past ~64 — `gerstnerHeight` / `gerstnerDisp` unroll over `waveConsts.length` and `foamAccumulator` re-invokes them at 4 past time samples, making the shader exceed driver-compile budgets. Kept `topK = 32`. The FFT-path's `foamAccumulator` is now also short-circuited (Jacobian + slope foam carry the work), which makes future topK bumps safer. Swell/chop sliders retained (they still drive the Gerstner amplitudes; no-op on spectrum). |
| A6 — Retire `?water=v2` after burn-in | 🔶 partial | **Default flipped** in the perf-audit PR — `?water` now defaults to `fft`, `?fftbake` defaults to `fft`, `?waves` defaults to spectrum. Legacy paths still reachable via `?water=v2`, `?fftbake=ddft`, `?waves=gerstner`. Gerstner code stays in place for the foreseeable future as the fallback — full removal is deferred until the new default has lived through enough playtests on every track + time of day that nobody's reaching for `?water=v2` anymore. |
...
| A8 — Foam feedback buffer (persistent trails + advection) | ✅ done | `ocean-fft/foam-feedback.ts` — A TSL compute kernel maintains a world-space R32F storage texture (256×256 over a 200 m tile, REPEAT-wrapped) using a `max(prev·decay, instantFoam)` update per texel. `instantFoam` is `smoothstep(jHigh, jLow, min(J_cascade_i))` with bilinear sampling across each cascade's `.a` channel; defaults `(jHigh, jLow) = (-0.2, -0.8)` and `decay = 0.93` (~700 ms half-life). Why those values: with the legacy `(0.5, 0.0)` near-breaking trigger, the per-frame max-feedback converges to ≈1 wherever the cascade Jacobian dips below 0.5 — saturating the buffer into a milky surface. Tightening the trigger to only "actually folding" texels (J ≤ -0.2 .. -0.8) keeps foam concentrated at real wave breaks, and the decay makes those breaks LEAVE TRAILS for ~700 ms instead of vanishing the moment the crest moves on. Single read-write storage texture (per-texel self-update is race-free — each compute thread only touches its own texel; r32float is the WebGPU-guaranteed read_write format). Bilinear sampling of cascade `.a` is done manually with 4 textureLoad taps + 2D lerp since `texture()` would need implicit derivatives unavailable in compute. The kernel ticks AFTER all three cascade displacement kernels each frame so it reads fresh Jacobians. Wired into the FFT path's `foldFoamFft` in `water.ts`; on the non-FFT or non-WebGPU paths the legacy stateless smoothstep stays in place (handle is `null`). **Advection** (added in a follow-up): the self-read happens at a shifted texel `(px, py) − drift·dt`, with bilinear interpolation, so foam trails drift downwind with the surface flow rather than staying anchored. Drift defaults to 30 % of cascade-0 wind speed in cascade-0's wind direction (the textbook surface-drift coefficient); `applySpectrumParams` re-syncs drift on every wind-speed/direction slider change so foam continues to flow in the current wind direction. Race-free because WGSL guarantees other invocations' writes in the same dispatch are not visible to reads. **Debug menu**: `Foam persistence` slider (0..1) maps to decay in `[0.7, 0.99]` so users can dial in fade speed live; storage bumped v4→v5. **URL escape hatch**: `?foamfb=0` disables the feedback handle for A/B comparison against the pre-A8 stateless look. **Validation done**: Chrome MCP A/B on the lagoon track at sunset palette confirms (a) clean cyan-green water with persistent foam trails behind passing crests when enabled, (b) faster-fading stateless foam when `?foamfb=0`, (c) the persistence slider live-mutates foam fade speed (verified at slider=0.0 → foam shrinks visibly; slider=0.95 → trails extend), (d) the non-FFT `?water=v2` path is unaffected, (e) 257/257 unit tests still pass, (f) typecheck clean. **Open follow-ups**: visual tune on remaining tracks (dune-rally, oval, the desert+water mix in big-bay). The A8 trigger thresholds may need per-track adjustment if some tracks have markedly different sea state. A jHigh/jLow slider pair could expose the trigger as a tuning knob (currently fixed). |

URL flags: see the table at the top. **The full FFT path is now the
default**; `?water=v2&waves=gerstner&fftbake=ddft` reverts to the
previous shipping look. **A8 escape hatch**: append `&foamfb=0` to the
URL to disable the persistent foam feedback for A/B comparison against
the pre-A8 stateless `foldFoamFft`.
...
- Per-track `water.wind { speed, dirX, dirZ, fetch }` schema. Currently all tracks use `defaultSpectrumParams()`. Adding per-track override would let lagoon read calmer than dune-rally or wherever, and would also be the natural home for per-track A8 foam thresholds. The wind-speed / direction / cutoff sliders that just landed are wired through `applySpectrumParams`, which is the same path a per-track override would use — so this is mostly schema + boot-time loading work, not new orchestration.
- A8 follow-ups: foam-persistence (decay) + (jHigh, jLow) thresholds as debug-menu sliders for per-session tuning; foam advection (drift with wave group velocity) in the kernel.
