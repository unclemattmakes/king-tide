...
| A5 — Tuning + debug menu rewrite | 🟡 partial | **Landed:** Choppiness (λ), Sea state (renderScale), and Wind speed sliders. Choppiness + Sea state live-mutate the GPU kernel's uniforms via `gpuDisplacementHandle.setChoppiness` / `setRenderScale`. Wind speed kicks off a live spectrum rebuild via `applySpectrumParams` in `water.ts` — `buildPhillipsSpectrum` (CPU, ~0.3 ms at N=32) + `selectTopKModes` (CPU buoyancy stays in sync) + `gpuDisplacementHandle.uploadSpectrum(grid)` (in-place repack of the spectrum DataTexture). Persisted to `hoverbike.waterDebug.v3` localStorage (old `v1` / `v2` keys auto-merge onto defaults). Visual A/B confirmed all three knobs visibly affect the FFT-path surface in real time. **Still open:** wind DIRECTION + small-wavelength cutoff sliders — same orchestration pattern as wind speed; same `applySpectrumParams` path with different params keys. Direction is two-axis (windDirX, windDirZ) and needs either a compass-style picker or two linked sliders. Swell/chop sliders are retained (they still drive the Gerstner amplitudes; no-op on spectrum). |
