# Status changelog archive (pre-2026-06-17 + v1-historical)

> This is the append-only running changelog moved out of
> [status.md](status.md) so that file stays a one-screen read-first doc.
> Everything here is preserved **verbatim**; entries are newest-first, the
...
> **ON by default** (foam oil-strokes already were; the water readability
...
> (nitro-deck iGPU, mexico-city worst case, 720p, 8 bikes): high 66.8 fps /
...
> mexico-city joined `BENCH_TRACKS`/profile defaults. **Findings (nitro-deck
