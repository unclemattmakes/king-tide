...
| **2. Track production sprint 1** | M13 | Sandbar (tutorial), South Beach, Hatteras, The Maw, Cape Town. Reef Cup + the hero track. Track-select tiles activate one by one. | 🟡 4/5 — Sandbar / South Beach Sunken / Hatteras Light / Cape Town Drift all build + export end-to-end via `pnpm seed:track-<id>` (GLB + JSON + hero/thumb JPGs shipped). The Maw still pending. Tiles stay disabled until each track flips to `status: 'ship'` in the catalog after art tuning. |
