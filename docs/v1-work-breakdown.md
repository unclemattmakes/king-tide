...
| **8. Polish / QA** | M17–M18 | Perf, accessibility, cross-browser. | 🟡 QA tooling (`pnpm qa` orchestrator + matrix + soak + bug-bundle + playbook + non-blocking CI), Perf HUD + perf-recorder, Accessibility tab (8 rows lit incl. colorblind palettes + reduced flash + screen-shake), cross-browser Playwright projects (chromium/firefox/webkit gated by `E2E_BROWSERS`), Steam Deck profile + Tauri 2 scaffold (Linux AppImage + Windows NSIS), mobile MENU button + touch HUD, water/underwater polish all landed. Perf-budget pass against the 60 fps / 8-bike target, colorblind palette playtest, and per-track asset streaming still pending. |
