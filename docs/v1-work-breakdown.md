...
| Leaderboard | ✅ TT PB submits to local board (deduped by handle) | ✅ gameplay → submit-times toggle + leaderboard-handle text row | ✅ menu → LEADERBOARDS opens top-N view |
...
| **6. Modes** | M16 | Time Trial + Cup wiring + leaderboard backend. Mode buttons activate. | 🟡 Time Trial + ghost recording shipped; cup wiring landed via Dev Placeholder Cup; local leaderboard board + submit toggle + handle row shipped (backend swap is still pending in M16) |
