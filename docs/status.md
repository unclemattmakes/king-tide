...
> Last updated: 2026-05-09 (M9.38 planar water reflection — TSL `reflector()` node mirrors bikes / sky / terrain onto the water with wave-normal distortion, Fresnel-mixed; `?reflect=0` and `?water=classic` keep the fresnelEmissive sky-tint fallback for A/B. M9.28 trimesh tunneling fix — bike body has CCD enabled, spec-driven track surfaces author as 1m slabs instead of 0-thickness planes; build_track.py now also emits `public/tracks/<id>.json` with start yaw + Catmull-Rom anchors so a single `pnpm gen:tracks` produces a fully playable track. Bike kit now edits-in-context: parts laid out at assembled positions in `bike_parts.blend`, attachment points are author-controlled `mount_*` empties parented to `chassis_base` (variants parented under each mount → moving a mount drags geometry live), one `Bike: <name>` snapshot per spec mirrors the in-game `?viewer=<id>` switch UX, and `geometry.chassisVariant` opts a spec into bespoke `chassis_<variant>` geometry instead of stretching the generic cube). Live build: https://hoverbike-ciaqaossl-oddballcreatureclubs-projects.vercel.app — every push to `main` auto-deploys.
