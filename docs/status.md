...
> Last updated: 2026-05-10 (M10.10 deployed PartyKit endpoint — client defaults to `hoverbike.occ-matt.partykit.dev` in prod builds, `localhost:1999` in dev; `?host=` overrides. Vercel + `pnpm party:deploy` now share the same relay so multiplayer works on the live URL without flags. M10.4–M10.9 multiplayer foundation — InputFrame wire format (10-byte LE record, src/engine/net/input-frame.ts), first PartyKit relay (party/relay.ts, peer-slot assignment, JSON control + binary frames), per-peer simulateStep via new PeerControlled component, remote-intent drain into per-tick sim inputs, remote-peer bikes spawn/despawn on peer-joined/peer-left and now Racer-tagged so they show in the position HUD, plus a top-left `#hud-room` chip surfacing room id + peer slot + remote peers. Two browser tabs at `?room=<id>` now see each other's bikes drive AND race in real time, each with a deterministic peer-coloured livery. Local dev: `pnpm party:dev` (PartyKit on :1999) + `pnpm dev` (Vite on :5191). M9.41 lean crank — `ROLL_LEAN_LIMIT` doubled from 20° to 40°, so the high-speed boost takes the bike to ~60° at top speed; the bike now visibly puts a knee down through the apex.). Live build: https://hoverbike-ciaqaossl-oddballcreatureclubs-projects.vercel.app — every push to `main` auto-deploys.
...
- **Multiplayer foundation (M10.4–M10.9)** — opt-in via `?room=<id>` URL param. Local dev needs `pnpm party:dev` (PartyKit relay on :1999) alongside `pnpm dev` (Vite on :5191). When connected to a room, every other tab on the same room id appears as a peer-controlled bike that drives in response to that peer's keyboard / gamepad. Inputs flow as 10-byte InputFrames; the relay (`party/relay.ts`) is stateless and assigns peer slots 0..7. Bikes for remote peers spawn on `peer-joined`, despawn on `peer-left`, are Racer-tagged so they show in the position HUD, and get a deterministic peer-id → livery colour. A `#hud-room` chip at the top-left shows the room id, your slot, and remote peers. The `__hover.net` devtools probe surfaces `peerId()`, `remotePeers()`, `recentRemoteFrames()`, and `latestPeerIntents()` for inspection. **Not yet implemented**: variant negotiation per peer (everyone uses the racer default GLB), shared / authoritative race state (each peer's standings are local-sim views), jitter buffer / tick alignment.
