...
> Last updated: 2026-05-10 (M10.4–M10.7 multiplayer foundation — InputFrame wire format (10-byte LE record, src/engine/net/input-frame.ts), first PartyKit relay (party/relay.ts, peer-slot assignment, JSON control + binary frames), per-peer simulateStep via new PeerControlled component, remote-intent drain into per-tick sim inputs, and remote-peer bikes spawn/despawn on peer-joined/peer-left. Two browser tabs at `?room=<id>` now see each other's bikes drive in real time. Local dev: `pnpm party:dev` (PartyKit on :1999) + `pnpm dev` (Vite on :5191). M9.41 lean crank — `ROLL_LEAN_LIMIT` doubled from 20° to 40°, so the high-speed boost takes the bike to ~60° at top speed; the bike now visibly puts a knee down through the apex.). Live build: https://hoverbike-ciaqaossl-oddballcreatureclubs-projects.vercel.app — every push to `main` auto-deploys.
...
- **Multiplayer foundation (M10.4–M10.7)** — opt-in via `?room=<id>` URL param. Local dev needs `pnpm party:dev` (PartyKit relay on :1999) alongside `pnpm dev` (Vite on :5191). When connected to a room, every other tab on the same room id appears as a peer-controlled bike that drives in response to that peer's keyboard / gamepad. Inputs flow as 10-byte InputFrames; the relay (`party/relay.ts`) is stateless and assigns peer slots 0..7. Bikes for remote peers spawn on `peer-joined` and despawn on `peer-left`; they are visible avatars only (no shared race result yet). The `__hover.net` devtools probe surfaces `peerId()`, `remotePeers()`, `recentRemoteFrames()`, and `latestPeerIntents()` for inspection. **Not yet implemented**: variant negotiation per peer, shared race state, jitter buffer / tick alignment, deployed PartyKit endpoint (currently localhost-only).
...
| M10.7 | Remote-peer bike spawn — `peer-joined` spawns a PeerControlled bike at a grid offset (default racer variant); `peer-left` despawns (removes rigid body + entity). Bike automatically receives relayed inputs via M10.5/M10.6. Visible avatars only — not yet Racer-tagged. | ✅ |
