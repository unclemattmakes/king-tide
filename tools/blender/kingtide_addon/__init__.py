"""King Tide — in-Blender "Export to Game" addon.
...
underway):
  ...
  kingtide_addon/
    ...
    _legacy.py     — Everything that used to be in
                     kingtide_addon.py, untouched. Subsequent PRs will
...
bl_info = {
    "name": "King Tide: Export to Game",
    "author": "King Tide",
    "version": (2, 0, 0),
    "blender": (3, 6, 0),
    "location": "View3D > Sidebar > King Tide",
    "description": "One-click export of bikes and tracks from Blender to the running King Tide game.",
...
_MODULES = (
    ...
    # run.
    panel,
    # prop_bake's KINGTIDE_PT_props_library_bake sub-panel parents on
    # KINGTIDE_PT_panel (in panel.py), so it MUST register after
    # panel. The save_pre handler it also registers is order-agnostic.
    prop_bake,
    # biome_palette's KINGTIDE_PT_track_biome_palette also parents on
    # KINGTIDE_PT_panel — register after panel for the same reason.
    # Order vs prop_bake is arbitrary; keep panel-children grouped here.
    biome_palette,
    # scatter_stroke's KINGTIDE_PT_track_scatter_stroke is another
    # panel child — same registration constraint, register after panel.
    scatter_stroke,
    # prop_placements adds the KINGTIDE_PT_props sub-panel parented on
    # KINGTIDE_PT_panel — another panel child, so it registers after panel.
    prop_placements,
    # propline_placements adds the KINGTIDE_PT_prop_lines sub-panel, also a
    # child of KINGTIDE_PT_panel — register after panel for the same reason.
    propline_placements,
...
# ``kingtide_addon.py`` file and ``seed_template_*.py`` /
