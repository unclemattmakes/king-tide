...
``tools/blender/kingtide_addon.py``; the refactor split user-facing
...
def _looks_like_clone(path: str | None) -> bool:
    """A king-tide clone has both ``package.json`` and ``public/``."""
    return bool(
        path
...
def _configured_repo_root() -> str | None:
    ...
    *outside* the clone (e.g. a Google-Drive-synced ``tracks-src/`` — see
    docs/asset-storage.md). Checks ``$KINGTIDE_REPO_ROOT`` first, then the
    ...
    env = os.environ.get("KINGTIDE_REPO_ROOT")
    if env:
...
def find_repo_root(start: str | None) -> str | None:
    """The king-tide clone that exports get written into (``public/`` +
    ``specs/`` live there). Resolution order:

    1. An explicit override — ``$KINGTIDE_REPO_ROOT`` or the add-on's
...
# operator live in kingtide_addon/previews.py — they share the
...
# ── Water (moved) ──────────────────────────────────────────────────────────
#
# Water authoring + preview live in `kingtide_addon/water.py`. Constants
...
#
# Turn-indicator chevrons live in `kingtide_addon/turn_indicators.py`.
...
# ── Ghost lap (moved) ──────────────────────────────────────────────────────
#
# Ghost-lap overlay + chase cam live in `kingtide_addon/ghost_lap.py`.

# ── Heightmap import + terrain sculpt (moved) ──────────────────────────────
#
# Both live in `kingtide_addon/terrain.py`.
...
# `kingtide_addon/spline.py`.


# ── Placement helper (moved) ───────────────────────────────────────────────
#
# Placement-helper empty + ramp/boost-pad attach ops live in
# `kingtide_addon/placement_helper.py`. The constant is re-exported so the
...
# `kingtide_addon/downtown.py`.



# ── Snap spline + gate preview OTs (moved) ────────────────────────────────
#
# KINGTIDE_OT_snap_spline_to_terrain, _rebuild_gate_preview, and
# _hide_gate_preview moved to kingtide_addon/previews.py.


# ── Terrain attribute bakers (moved) ───────────────────────────────────────
#
# AO + path-wear bake live in `kingtide_addon/bake.py`.



# ── Track stats + lint (moved) ─────────────────────────────────────────
#
# Track-introspection operators (KINGTIDE_OT_refresh_track_stats,
# KINGTIDE_OT_lint_track) live in kingtide_addon/track_meta.py.
...
# reload OT live in kingtide_addon/export.py. Validation, JSON
...
# live in kingtide_addon/handlers.py. Property update callbacks in
...
#
# All UI panel classes live in kingtide_addon/panel.py.
