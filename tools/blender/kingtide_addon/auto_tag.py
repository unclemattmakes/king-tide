...
``kingtide.retag_scene`` operator so authors can force a re-run after
bulk renames.
...
_RULES: tuple[_AutoTagRule, ...] = (
    ...
    # ``kingtide_addon/emitter.py`` exactly — the rule re-stamps them
...
class KINGTIDE_OT_retag_scene(Operator):
    ...
    bl_idname = "kingtide.retag_scene"
...
_CLASSES: tuple[type, ...] = (KINGTIDE_OT_retag_scene,)
