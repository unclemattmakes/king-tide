...
export scrubs, so none of it reaches the runtime.
  ...
  * ``KINGTIDE_OT_rebuild_gate_preview`` / ``KINGTIDE_OT_hide_gate_preview``
    — drop gate gizmos every gateSpacing metres along ai_spline_main.
  * ``KINGTIDE_OT_rebuild_racer_preview`` / ``KINGTIDE_OT_hide_racer_preview``
    — place a bike silhouette at ``start_00`` plus each AI grid slot.
  * ``KINGTIDE_OT_snap_spline_to_terrain``
...
class KINGTIDE_OT_rebuild_gate_preview(Operator):
    ...
    bl_idname = "kingtide.rebuild_gate_preview"
...
class KINGTIDE_OT_rebuild_racer_preview(Operator):
    ...
    bl_idname = "kingtide.rebuild_racer_preview"
...
class KINGTIDE_OT_hide_racer_preview(Operator):
    """Toggle the racer-preview collection's visibility off without
    deleting it. Re-run Rebuild to bring it back."""

    bl_idname = "kingtide.hide_racer_preview"
...
class KINGTIDE_OT_rebuild_rider_preview(Operator):
    ...
    bl_idname = "kingtide.rebuild_rider_preview"
...
class KINGTIDE_OT_hide_rider_preview(Operator):
    """Remove the rider preview (its collection + imported rig/clips)."""

    bl_idname = "kingtide.hide_rider_preview"
...
class KINGTIDE_OT_reload_props_library(Operator):
    ...
    King Tide library, then rebuilds the gate preview so the new mesh
    is reflected in the viewport gizmos. Idempotent; safe to spam."""

    bl_idname = "kingtide.reload_props_library"
    ...
    def execute(self, context):
        ...
        if not reloaded:
            self.report(
                {"INFO"},
                "No King Tide libraries linked in this scene. "
...
class KINGTIDE_OT_snap_spline_to_terrain(Operator):
    ...
    bl_idname = "kingtide.snap_spline_to_terrain"
...
_CLASSES: tuple[type, ...] = (
    KINGTIDE_OT_rebuild_gate_preview,
    KINGTIDE_OT_hide_gate_preview,
    KINGTIDE_OT_rebuild_racer_preview,
    KINGTIDE_OT_hide_racer_preview,
    KINGTIDE_OT_rebuild_rider_preview,
    KINGTIDE_OT_hide_rider_preview,
    KINGTIDE_OT_reload_props_library,
    KINGTIDE_OT_snap_spline_to_terrain,
)
