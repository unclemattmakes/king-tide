...
def _candidate_seed_paths() -> list[str]:
    ...
    try:
        ...
        # __file__ → .../tools/blender/kingtide_addon/island_terrain.py
...
class KINGTIDE_OT_add_island_terrain(Operator):
    ...
    * Skips peak empties that already exist with the expected names.
    ...
    bl_idname = "kingtide.add_island_terrain"
    ...
    def execute(self, context: bpy.types.Context) -> set[str]:
        ...
        if seed is None:
            self.report(
                ...
                "inside a king-tide clone, or re-run `pnpm install:blender-addon` "
...
class KINGTIDE_OT_add_island_mod_zone(Operator):
    ...
    * ``scale.x`` — zone radius in metres (smoothstep falloff to 0).
    ...
    bl_idname = "kingtide.add_island_mod_zone"
...
_CLASSES: tuple[type, ...] = (
    KINGTIDE_OT_add_island_terrain,
    KINGTIDE_OT_add_island_mod_zone,
)
