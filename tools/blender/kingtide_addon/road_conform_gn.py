...
The classic flow in ``road.py`` (``KINGTIDE_OT_build_road``) carves
...
* :class:`KINGTIDE_OT_attach_road_conform` adds (or refreshes) that
  modifier on the active terrain and wires ``road_curve_main`` into
  the modifier's curve input socket.

* :class:`KINGTIDE_OT_snap_curve_to_terrain` is the one-shot seeder.
...
Trade-offs vs. the destructive ``KINGTIDE_OT_build_road`` carve:
...
def build_road_conform_node_group() -> bpy.types.NodeTree:
    ...
    # See tools/blender/seed_template_island.py for the full rationale.
    g.is_modifier = True
...
class KINGTIDE_OT_attach_road_conform(Operator):
    ...
    bl_idname = "kingtide.attach_road_conform"
...
class KINGTIDE_OT_snap_curve_to_terrain(Operator):
    ...
    bl_idname = "kingtide.snap_curve_to_terrain"
...
_CLASSES: tuple[type, ...] = (
    KINGTIDE_OT_attach_road_conform,
    KINGTIDE_OT_snap_curve_to_terrain,
)
