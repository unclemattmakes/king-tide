...
def _find_repo_root() -> str | None:
    ...
    king-tide clone — the operator surfaces a clear error in that case
...
def _template_enum_items(self, context):
    ...
    if repo is None:
        return [("__none__", "(no repo)", "Open a .blend inside the king-tide repo first")]
...
class KINGTIDE_OT_new_map_from_template(Operator):
    ...
    bl_idname = "kingtide.new_map_from_template"
    ...
    def invoke(self, context, event):
        ...
        if repo is None:
            self.report(
                {"ERROR"},
                "Open or save any .blend inside the king-tide repo first — "
...
_CLASSES: tuple[type, ...] = (KINGTIDE_OT_new_map_from_template,)
