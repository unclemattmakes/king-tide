...
under the "King Tide" tab — live here. The parent ``KINGTIDE_PT_panel``
...
up. The top-bar King Tide menu (``menu.py``) is the always-available
...
# KINGTIDE_PT_track_emitters lives in emitter.py (panel draw uses
# module-local helpers there) but registers via this module so its
# parent panel KINGTIDE_PT_panel is in bpy.types first — panel runs
# last in addon._MODULES.
from .emitter import KINGTIDE_PT_track_emitters
from .decal import KINGTIDE_PT_track_decals
...
class KINGTIDE_PT_panel(Panel):
    bl_label = "King Tide"
    bl_idname = "KINGTIDE_PT_panel"
    ...
    def _draw_track(self, context, layout, blend: str, repo: str | None) -> None:
        ...
        UI lives in sub-panels (KINGTIDE_PT_track_*) so authors can
        ...
        if repo:
            ...
            warn_row = layout.row()
            warn_row.alert = True
            warn_row.label(text="Save .blend inside a king-tide clone", icon="ERROR")
        ...
        if essentials_missing:
            ...
            row.scale_y = 1.4
            row.operator(
                "kingtide.scaffold_track_essentials",
        ...
        if have_sp:
            ...
            if arc_m > 0:
                ...
                if clip_hits:
                    ...
                    stat_box.operator(
                        "kingtide.shift_spline_off_obstacles",
        ...
        row = layout.row()
        row.scale_y = 1.6
        row.operator("kingtide.export_track", icon="EXPORT")
        layout.prop(
        ...
        row.operator("kingtide.lint_track", icon="CHECKMARK")
        row.operator("kingtide.reload_track_json", text="Reload JSON", icon="FILE_REFRESH")
        row = layout.row(align=True)
        op_play_open = row.operator("kingtide.open_play_url", text="Play", icon="PLAY")
        op_play_open.edit = False
        op_edit_open = row.operator("kingtide.open_play_url", text="Edit", icon="GREASEPENCIL")
        op_edit_open.edit = True
        row = layout.row(align=True)
        op_play = row.operator("kingtide.copy_track_url", text="Copy Play", icon="URL")
        op_play.edit = False
        op_edit = row.operator("kingtide.copy_track_url", text="Copy Edit", icon="URL")
        op_edit.edit = True
        ...
        if active_kind is not None:
            ...
            if not have_terrain:
                col.operator(
                    "kingtide.add_multibiome_terrain",
                    text="Add Terrain (Style menu)",
                    icon="RNDCURVE",
                )
                col.operator(
                    "kingtide.add_island_terrain",
                    text="Add Island Terrain",
                    icon="RNDCURVE",
                )
            col.operator(
                "kingtide.add_placement_helper",
                text="Add Placement Helper",
                icon="EMPTY_ARROWS",
            )
            col.operator(
                "kingtide.rebuild_gate_preview",
                text="Rebuild Gate Preview",
                icon="MOD_ARRAY",
            )
            col.operator(
                "kingtide.new_map_from_template",
                text="New Map from Template…",
                icon="FILE_NEW",
            )
            sub = ebox.row()
            sub.scale_y = 0.85
            sub.label(text="All tools: top-bar King Tide menu", icon="MENU_PANEL")
    ...
    def _draw_bike(self, context, layout, blend: str, repo: str | None) -> None:
        ...
        if repo:
            ...
            box.label(text="Save .blend inside a king-tide/ clone.")

        row = layout.row()
        row.scale_y = 1.6
        row.operator("kingtide.export_bike", icon="EXPORT")

        col = layout.column(align=True)
        op_play = col.operator(
            "kingtide.copy_bike_url", text="Copy Play URL", icon="URL"
        )
        op_play.viewer = False
        op_view = col.operator(
            "kingtide.copy_bike_url", text="Copy Viewer URL", icon="HIDE_OFF"
        )
        op_view.viewer = True
        ...
        if bpy.data.objects.get("socket_seat") is None:
            ...
            row.operator(
                "kingtide.rebuild_rider_preview", text="Add Rider", icon="OUTLINER_OB_ARMATURE"
            )
            row.operator("kingtide.hide_rider_preview", text="", icon="X")
    ...
    def _draw_rider(self, context, layout, blend: str, repo: str | None) -> None:
        ...
        row = layout.row()
        row.scale_y = 1.6
        row.operator("kingtide.export_rider_pose", icon="EXPORT")
        ...
        refs.operator("kingtide.refresh_bike_refs", icon="FILE_REFRESH")
        ...
        seat.operator("kingtide.copy_seat_offset", icon="COPYDOWN")
...
# ────────────────────────────────────────────────────────────────────
#
# Each sub-panel is a child of KINGTIDE_PT_panel and only renders in
...
# matter of subclassing _KingTideTrackSubPanelBase + implementing draw.
...
class _KingTideTrackSubPanelBase:
    """Mixin: panel constants + poll() shared by every track sub-panel.
    Sub-panels live under the parent `KINGTIDE_PT_panel` and only render
...
class _SelectionDrivenPanel(_KingTideTrackSubPanelBase):
...
class KINGTIDE_PT_track_spline(_SelectionDrivenPanel, Panel):
    ...
    bl_idname = "KINGTIDE_PT_track_spline"
    ...
    def draw(self, context):
        ...
        if not (have_sp and have_s0 and have_s1):
            ...
            if not have_sp:
                srow.operator(
                    "kingtide.add_ai_spline", text="Add Spline", icon="CURVE_NCURVE",
                )
            if not (have_s0 and have_s1):
                srow.operator(
                    "kingtide.add_starts", text="Add Starts", icon="EMPTY_ARROWS",
                )
        ...
        row = layout.row(align=True)
        row.operator("kingtide.snap_spline_to_terrain", icon="SNAP_FACE")
        row.operator("kingtide.reverse_spline", text="Reverse", icon="LOOP_BACK")
        ...
        row.operator("kingtide.cursor_snap_to_spline", text="Cursor", icon="PIVOT_CURSOR")
        row = layout.row(align=True)
        row.prop(scene, "hoverbike_start_grid_spacing", text="Start gap")
        row.operator("kingtide.snap_starts_to_spline", text="Snap Starts", icon="EMPTY_ARROWS")
        layout.operator("kingtide.add_ramp_at_spline_t", text="Add Ramp at t", icon="ADD")


class KINGTIDE_PT_track_start(_SelectionDrivenPanel, Panel):
    ...
    bl_label = "Start gate"
    bl_idname = "KINGTIDE_PT_track_start"
    ...
    def draw(self, context):
        ...
        if bound:
            ...
            row = layout.row(align=True)
            row.operator(
                "kingtide.unbind_start_from_spline",
            ...
            row.prop(scene, "hoverbike_start_backoff_m", text="Back-off")
            row.operator(
                "kingtide.snap_starts_to_spline",
                text="Snap (once)",
                icon="EMPTY_ARROWS",
            )
        elif not have_sp:
        ...
        row = layout.row(align=True)
        row.operator(
            "kingtide.rebuild_racer_preview",
            text="Rebuild Grid",
            icon="FILE_REFRESH",
        )
        row.operator(
            "kingtide.hide_racer_preview",
            text="Hide",
            icon="HIDE_ON",
        )


class KINGTIDE_PT_track_road(_SelectionDrivenPanel, Panel):
    """Sub-panel: road-curve authoring + width / banking / curb knobs +
    Build Road. Visible when ``road_curve_main`` or a road mesh is the
    active object."""
    bl_label = "Road tool"
    bl_idname = "KINGTIDE_PT_track_road"
    ...
    def draw(self, context):
        ...
        layout = self.layout
        scene = context.scene
        layout.operator("kingtide.add_road_starter_curve", icon="CURVE_BEZCURVE")
        row = layout.row(align=True)
        ...
        row = layout.row(align=True)
        row.operator("kingtide.mark_selected_floating", text="Float", icon="ORIENTATION_VIEW")
        row.operator("kingtide.mark_selected_conforming", text="Conform", icon="OUTLINER_OB_FORCE_FIELD")
        ...
        row = layout.row(align=True)
        row.operator(
            "kingtide.snap_curve_to_terrain",
            text="Snap Curve",
            icon="SNAP_ON",
        )
        row.operator(
            "kingtide.attach_road_conform",
            text="Attach Conform",
            icon="GEOMETRY_NODES",
        )
        row = layout.row(align=True)
        row.scale_y = 1.3
        row.operator("kingtide.build_road", icon="MESH_PLANE")
        ...
        row = layout.row(align=True)
        row.operator(
            "kingtide.bake_terrain_to_road",
            text="Bake Terrain to Road",
            icon="MOD_SHRINKWRAP",
        )
        row.operator(
            "kingtide.reconform_terrain_to_road",
            text="Re-conform",
            icon="MOD_SHRINKWRAP",
        )


class KINGTIDE_PT_track_tunnels(_SelectionDrivenPanel, Panel):
    ...
    the active object."""
    bl_label = "Tunnels"
    bl_idname = "KINGTIDE_PT_track_tunnels"
    ...
    def draw(self, context):
        ...
        if in_edit_mode:
            edit_row.alert = True
            edit_row.operator(
                "kingtide.toggle_tunnel_edit_mode",
                text="Re-attach Cuts",
                icon="MOD_BOOLEAN",
            )
        else:
        ...
        layout.separator()
        layout.operator("kingtide.add_tunnel_starter_curve", icon="CURVE_BEZCURVE")
        row = layout.row(align=True)
        ...
        row = layout.row()
        row.scale_y = 1.3
        row.operator("kingtide.build_tunnel", icon="MESH_CYLINDER")
        n_tunnels = sum(
...
class KINGTIDE_PT_track_placement(_SelectionDrivenPanel, Panel):
    ...
    Create one via the King Tide → Add menu (or Utility → Placement
    Helper)."""
    bl_label = "Placement helper"
    bl_idname = "KINGTIDE_PT_track_placement"
    ...
    def draw(self, context):
        ...
        row = layout.row(align=True)
        if helper is None:
            row.operator("kingtide.add_placement_helper", icon="EMPTY_ARROWS")
        else:
            row.operator("kingtide.add_placement_helper", text="Re-pose Helper", icon="FILE_REFRESH")
            row.operator("kingtide.remove_placement_helper", text="", icon="X")
            layout.label(text=f"@ {helper.location.x:+.1f}, {helper.location.y:+.1f}, {helper.location.z:+.1f}",
                         icon="OBJECT_DATAMODE")
            layout.separator()
            layout.label(text="One-click drop:")
            layout.operator("kingtide.cursor_to_helper", icon="PIVOT_CURSOR")
            row = layout.row(align=True)
            row.operator("kingtide.add_ramp_at_helper", icon="ADD")
            row.operator("kingtide.add_boost_pad_at_helper", icon="FORCE_FORCE")


class KINGTIDE_PT_track_downtown(_SelectionDrivenPanel, Panel):
    ...
    object. Use King Tide → Add → Downtown Block to drop a fresh one."""
    bl_label = "Downtown"
    bl_idname = "KINGTIDE_PT_track_downtown"
    ...
    def draw(self, context):
        ...
        layout.operator("kingtide.add_downtown", icon="MESH_CUBE")
        ...
        if active_dt is not None:
            ...
            row = layout.row(align=True)
            row.operator("kingtide.pick_downtown_settings", text="Pick Settings", icon="EYEDROPPER")
            row.operator("kingtide.rebuild_downtown", text="Rebuild", icon="FILE_REFRESH")


class KINGTIDE_PT_track_antigrav_ribbon(_SelectionDrivenPanel, Panel):
    ...
    is the active object. Use King Tide → Add → Anti-Grav Curve to
    spawn the first one."""
    bl_label = "Anti-grav surfaces"
    bl_idname = "KINGTIDE_PT_track_antigrav_ribbon"
    ...
    def draw(self, context):
        ...
        row = layout.row(align=True)
        row.operator("kingtide.add_antigrav_curve", icon="CURVE_BEZCURVE")
        row.operator("kingtide.build_antigrav_surface", icon="MOD_PARTICLES")
...
class KINGTIDE_PT_track_ramps(_SelectionDrivenPanel, Panel):
    ...
    King Tide → Add → Ramp is the spawn entry."""
    bl_label = "Ramps"
    bl_idname = "KINGTIDE_PT_track_ramps"
    ...
    def draw(self, context):
        ...
        row = layout.row()
        row.scale_y = 1.3
        row.operator("kingtide.add_ramp", icon="ADD")


class KINGTIDE_PT_track_terrain(_SelectionDrivenPanel, Panel):
    ...
    King Tide → Terrain → Import Heightmap creates the first one."""
    bl_label = "Terrain"
    bl_idname = "KINGTIDE_PT_track_terrain"
    ...
    def draw(self, context):
        ...
        layout.operator("kingtide.import_heightmap", icon="IMPORT")
        layout.separator()

        layout.label(text="Sculpt:", icon="SCULPTMODE_HLT")
        layout.operator("kingtide.apply_terrain_modifiers", icon="MODIFIER_DATA")
        layout.operator("kingtide.subdivide_terrain", icon="MOD_SUBSURF")
        layout.operator("kingtide.enter_sculpt_mode", icon="BRUSH_DATA")
        layout.separator()
        ...
        row = layout.row(align=True)
        op_up = row.operator("kingtide.raise_lower_terrain", text="Raise", icon="TRIA_UP")
        op_up.lower = False
        op_dn = row.operator("kingtide.raise_lower_terrain", text="Lower", icon="TRIA_DOWN")
        op_dn.lower = True
        layout.separator()
        ...
        layout.operator("kingtide.smooth_terrain", icon="MOD_SMOOTH")
        layout.separator()
        ...
        if find_island_modifier(context.active_object) is None:
            layout.operator("kingtide.apply_terrain_vertex_colors", icon="COLOR")
        layout.label(text="Fills baked_ao + baked_path", icon="NODE_TEXTURE")
        layout.operator("kingtide.bake_terrain_attrs", icon="MOD_NOISE")
        ...
        layout.operator("kingtide.bake_path_worn", icon="MOD_CURVE")


class KINGTIDE_PT_track_water(_SelectionDrivenPanel, Panel):
    ...
    bl_label = "Water"
    bl_idname = "KINGTIDE_PT_track_water"
    ...
    def draw(self, context):
        ...
        row.operator("kingtide.fit_water_preview_to_scene", text="Fit", icon="FULLSCREEN_ENTER")
        row = layout.row(align=True)
        row.prop(scene, "hoverbike_water_subdivisions", text="Subdiv")
        row.prop(scene, "hoverbike_water_time", text="Time (s)")
        row = layout.row(align=True)
        row.operator("kingtide.rebuild_water_preview", icon="FILE_REFRESH")
        row.operator("kingtide.hide_water_preview", icon="HIDE_ON")
...
class KINGTIDE_PT_track_horizon(_SelectionDrivenPanel, Panel):
    ...
    Visible when ``horizon_ring`` is the active object. Use King Tide
    → Add → Horizon Ring to spawn the starter shape."""

    bl_label = "Horizon"
    bl_idname = "KINGTIDE_PT_track_horizon"
    ...
    def draw(self, context):
        ...
        if ring is None:
            ...
            layout.operator("kingtide.add_horizon_ring", icon="ADD")
        else:
            v = len(ring.data.vertices)
            f = len(ring.data.polygons)
            layout.label(text=f"horizon_ring: {v} verts, {f} faces", icon="WORLD")
            row = layout.row(align=True)
            row.operator("kingtide.edit_horizon_ring", icon="EDITMODE_HLT")
            row.operator("kingtide.delete_horizon_ring", icon="X")
            layout.separator()
            layout.label(text="Re-roll starter (loses edits):", icon="FILE_REFRESH")
            row = layout.row(align=True)
            row.prop(scene, "hoverbike_horizon_seed", text="Seed")
            row.prop(scene, "hoverbike_horizon_peak", text="Peak")
            layout.operator("kingtide.reset_horizon_ring", icon="LOOP_BACK")


class KINGTIDE_PT_track_sky(_KingTideTrackSubPanelBase, Panel):
    ...
    bl_label = "Sky preset"
    bl_idname = "KINGTIDE_PT_track_sky"
...
class KINGTIDE_PT_track_waves(_SelectionDrivenPanel, Panel):
    ...
    bl_label = "Wave zones"
    bl_idname = "KINGTIDE_PT_track_waves"
    _active_pred = staticmethod(_is_wave_zone_active)

    def draw(self, context):
        layout = self.layout
        layout.operator("kingtide.add_wave_zone", icon="ADD")
        n_zones = sum(
        ...
        if n_zones > 0:
            ...
            row.operator("kingtide.refresh_wave_zones", text="Refresh", icon="FILE_REFRESH")
        else:
            layout.label(text="No zones — global Gerstner only", icon="INFO")


class KINGTIDE_PT_track_gameplay(_SelectionDrivenPanel, Panel):
    ...
    preview, or a ``turn_arrow_NN``. Use King Tide → Add for spawn
    actions when nothing relevant is selected."""
    bl_label = "Gameplay"
    bl_idname = "KINGTIDE_PT_track_gameplay"
    ...
    def draw(self, context):
        ...
        row = layout.row(align=True)
        row.operator("kingtide.rebuild_gate_preview", icon="FILE_REFRESH")
        row.operator("kingtide.hide_gate_preview", icon="HIDE_ON")
        ...
        if n_cp > 0:
            ...
            row.operator(
                "kingtide.demote_gates_to_spline",
                text="Wipe",
                icon="X",
            )
            layout.operator(
                "kingtide.materialize_gates_to_cp_empties",
        ...
        layout.operator("kingtide.rebuild_buoys", icon="FILE_REFRESH")
        layout.separator()

        layout.label(text="Boost pads:", icon="FORCE_FORCE")
        layout.operator("kingtide.add_boost_pad", icon="ADD")
        n_pads = sum(
        ...
        if n_pads > 0:
            ...
            row.operator("kingtide.refresh_boost_pads", text="Refresh", icon="FILE_REFRESH")
        layout.separator()
        ...
        if sp is not None:
            ...
            row.operator("kingtide.toggle_spline_antigrav", text="Toggle")
        if context.mode == "EDIT_CURVE":
            row = layout.row(align=True)
            row.operator("kingtide.set_spline_tilt_flat", text="Flat")
            row.operator("kingtide.set_spline_tilt_bank_l", text="L 45°")
            row.operator("kingtide.set_spline_tilt_bank_r", text="R 45°")
            row = layout.row(align=True)
            row.operator("kingtide.set_spline_tilt_wall_l", text="Wall L")
            row.operator("kingtide.set_spline_tilt_wall_r", text="Wall R")
            row.operator("kingtide.set_spline_tilt_ceiling", text="Ceiling")
        layout.operator("kingtide.add_antigrav_zone", icon="ADD", text="+ Anti-Grav Zone")
        n_zones = sum(
        ...
        if n_zones > 0:
            ...
            row.operator("kingtide.refresh_antigrav_zones", text="Refresh", icon="FILE_REFRESH")
        layout.separator()

        layout.label(text="Racer preview:", icon="AUTO")
        row = layout.row(align=True)
        row.operator("kingtide.rebuild_racer_preview", icon="FILE_REFRESH")
        row.operator("kingtide.hide_racer_preview", icon="HIDE_ON")
        layout.separator()
        ...
        row = layout.row(align=True)
        row.operator("kingtide.rebuild_turn_indicators", icon="FILE_REFRESH")
        row.operator("kingtide.hide_turn_indicators", icon="HIDE_ON")


class KINGTIDE_PT_track_ghost(_KingTideTrackSubPanelBase, Panel):
    ...
    bl_idname = "KINGTIDE_PT_track_ghost"
    bl_options = {"DEFAULT_CLOSED"}
    ...
    def draw(self, context):
        ...
        row = layout.row(align=True)
        row.operator("kingtide.rebuild_ghost_lap", icon="FILE_REFRESH")
        row.operator("kingtide.hide_ghost_lap", icon="HIDE_ON")
        layout.label(text="Press Spacebar to fly the lap", icon="PLAY")


class KINGTIDE_PT_track_shader(_KingTideTrackSubPanelBase, Panel):
    ...
    bl_idname = "KINGTIDE_PT_track_shader"
    bl_options = {"DEFAULT_CLOSED"}
...
class KINGTIDE_PT_track_thumbnail(_KingTideTrackSubPanelBase, Panel):
    ...
    bl_idname = "KINGTIDE_PT_track_thumbnail"
    bl_options = {"DEFAULT_CLOSED"}
    ...
    def draw(self, context):
        ...
        row.scale_y = 1.3
        row.operator(
            "kingtide.render_track_hero",
            text="Render Hero",
            icon="RENDER_STILL",
        )
        row.operator(
            "kingtide.render_track_thumbnail",
            text="Tile only",
            icon="IMAGE",
        )
...
class KINGTIDE_PT_track_stats(_KingTideTrackSubPanelBase, Panel):
    ...
    bl_idname = "KINGTIDE_PT_track_stats"
    bl_options = {"DEFAULT_CLOSED"}
    ...
    def draw(self, context):
        ...
        layout.operator("kingtide.refresh_track_stats", icon="FILE_REFRESH")
        layout.separator()
        ...
        layout.operator("kingtide.retag_scene", icon="OUTLINER_DATA_FONT")
...
_CLASSES: tuple[type, ...] = (
    ...
    KINGTIDE_PT_track_spline,
    KINGTIDE_PT_track_start,
    KINGTIDE_PT_track_road,
    KINGTIDE_PT_track_tunnels,
    KINGTIDE_PT_track_antigrav_ribbon,
    KINGTIDE_PT_track_ramps,
    KINGTIDE_PT_track_downtown,
    KINGTIDE_PT_track_terrain,
    KINGTIDE_PT_track_water,
    KINGTIDE_PT_track_horizon,
    KINGTIDE_PT_track_waves,
    KINGTIDE_PT_track_gameplay,
    KINGTIDE_PT_track_placement,
    ...
    KINGTIDE_PT_track_emitters,
    KINGTIDE_PT_track_decals,
    KINGTIDE_PT_track_sky,
    KINGTIDE_PT_track_ghost,
    KINGTIDE_PT_track_shader,
    KINGTIDE_PT_track_thumbnail,
    KINGTIDE_PT_track_stats,
)
