...
* ``kingtide.apply_terrain_vertex_colors`` (the addon operator) — builds
...
def ensure_mat_terrain_main(*, rebuild: bool = False) -> bpy.types.Material:
    ...
    if mat is not None and not rebuild:
        ...
        if stored >= TERRAIN_MATERIAL_VERSION:
            return mat
        print(
            f"[kingtide] terrain material is v{stored}, addon is "
...
class KINGTIDE_OT_add_terrain_material(Operator):
    ...
    bl_idname = "kingtide.add_terrain_material"
...
_CLASSES: tuple[type, ...] = (
    KINGTIDE_OT_add_terrain_material,
)
