...
before exporting.
  ...
  * ``KINGTIDE_OT_refresh_track_stats`` — evaluates the terrain mesh
    and stashes min/max y + water-coverage onto scene props for the
    panel readout.
  * ``KINGTIDE_OT_lint_track`` — walks the spline, start, and terrain
...
class KINGTIDE_OT_refresh_track_stats(Operator):
    ...
    bl_idname = "kingtide.refresh_track_stats"
...
def _collect_obstacle_bboxes(
    ...
    *,
    ...
    (lint) and ``KINGTIDE_OT_shift_spline_off_obstacles`` (authoring
...
class KINGTIDE_OT_lint_track(Operator):
...
_CLASSES: tuple[type, ...] = (
    KINGTIDE_OT_refresh_track_stats,
    KINGTIDE_OT_lint_track,
)
