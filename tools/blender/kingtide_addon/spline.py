...
replace that ritual:
  ...
  * ``KINGTIDE_OT_cursor_snap_to_spline``  → moves the 3D cursor to a
    parameter t in [0, 1] on ``ai_spline_main`` (or another curve via
    scene prop), with rotation aligned to the racing tangent.
  * ``KINGTIDE_OT_snap_starts_to_spline``  → reposes ``start_00`` /
    ``start_01`` perpendicular to the racing tangent.
  * ``KINGTIDE_OT_add_ramp_at_spline_t``   → snaps the cursor, then
    immediately drops a ramp via ``kingtide.add_ramp``. One-click.
  * ``KINGTIDE_OT_auto_place_ramps``       → reuses the curvature-peak
...
operators promote a one-click fix to the panel:
  ...
  * ``KINGTIDE_OT_add_ai_spline``          → if the active object is a
...
class KINGTIDE_OT_cursor_snap_to_spline(Operator):
    ...
    bl_idname = "kingtide.cursor_snap_to_spline"
...
class KINGTIDE_OT_snap_starts_to_spline(Operator):
    ...
    bl_idname = "kingtide.snap_starts_to_spline"
    ...
    def execute(self, context):
        ...
        # Face the bike perpendicular to the starting line — i.e. along
        # the racing tangent at the anchor t. The gate plane sits
        # perpendicular to (tx, ty), so a bike heading (tx, ty) crosses
        # the line head-on, which is what we want from the grid.
        #
        # atan2(tx, -ty) (not the more obvious atan2(tx, ty)) is the
        # runtime-correct formula: Blender's +Y maps to three.js' -Z
        # forward, so the equivalent runtime atan2 is over (dx, -dy).
        # Skipping the ±π/2 correction in the previous fallback was the
        # reason the "nearest gate" aim path was added — but the gate-
        # aim heading drifted off-tangent whenever the spline curved
        # between the anchor and the first downstream gate, which is
        # exactly the regression Matt called out. Tangent-based yaw is
        # also independent of the gate-spacing slider so it stays
        # stable when authors tune spacing.
...
class KINGTIDE_OT_bind_start_to_spline(Operator):
    ...
    bl_idname = "kingtide.bind_start_to_spline"
    ...
    def execute(self, context):
        ...
        # picks up the new t (see KINGTIDE_OT_snap_starts_to_spline:
...
class KINGTIDE_OT_unbind_start_from_spline(Operator):
    ...
    bl_idname = "kingtide.unbind_start_from_spline"
...
class KINGTIDE_OT_add_ramp_at_spline_t(Operator):
    ...
    bl_idname = "kingtide.add_ramp_at_spline_t"
    ...
    def execute(self, context):
        ...
        snap = bpy.ops.kingtide.cursor_snap_to_spline()
        if snap != {"FINISHED"}:
            return snap
        return bpy.ops.kingtide.add_ramp()
...
class KINGTIDE_OT_materialize_gates_to_cp_empties(Operator):
    ...
    bl_idname = "kingtide.materialize_gates_to_cp_empties"
...
class KINGTIDE_OT_demote_gates_to_spline(Operator):
    ...
    bl_idname = "kingtide.demote_gates_to_spline"
...
class KINGTIDE_OT_reverse_spline(Operator):
    ...
    bl_idname = "kingtide.reverse_spline"
    ...
    def execute(self, context):
        ...
        if pre_reverse_anchor is not None:
            ...
            if new_t is not None:
                ...
                try:
                    if bpy.ops.kingtide.snap_starts_to_spline() == {"FINISHED"}:
...
class KINGTIDE_OT_shift_spline_off_obstacles(Operator):
    ...
    bl_idname = "kingtide.shift_spline_off_obstacles"
...
class KINGTIDE_OT_add_ai_spline(Operator):
    ...
    After the spline lands, the operator chains:
      ...
      * ``kingtide.add_starts``         → spawn / snap start_00 / 01
      * ``kingtide.rebuild_gate_preview`` → drop gate gizmos along the line
      * ``kingtide.rebuild_buoys``      → marker buoys over open water
    ...
    bl_idname = "kingtide.add_ai_spline"
    ...
    def _run_sidekicks(self) -> list[str]:
        ...
        for op_call, label in (
            (bpy.ops.kingtide.add_starts, "starts"),
            (bpy.ops.kingtide.rebuild_gate_preview, "gates"),
            (bpy.ops.kingtide.rebuild_buoys, "buoys"),
        ):
            try:
                ...
                print(f"[kingtide] add_ai_spline sidekick {label!r} skipped: {e}")
        return ran
...
class KINGTIDE_OT_add_starts(Operator):
...
class KINGTIDE_OT_scaffold_track_essentials(Operator):
    ...
    bl_idname = "kingtide.scaffold_track_essentials"
    ...
    def execute(self, context):
        ...
        bpy.ops.kingtide.add_ai_spline()
        bpy.ops.kingtide.add_starts()
...
_CLASSES: tuple[type, ...] = (
    KINGTIDE_OT_cursor_snap_to_spline,
    KINGTIDE_OT_snap_starts_to_spline,
    KINGTIDE_OT_bind_start_to_spline,
    KINGTIDE_OT_unbind_start_from_spline,
    KINGTIDE_OT_add_ramp_at_spline_t,
    KINGTIDE_OT_reverse_spline,
    KINGTIDE_OT_shift_spline_off_obstacles,
    KINGTIDE_OT_materialize_gates_to_cp_empties,
    KINGTIDE_OT_demote_gates_to_spline,
    KINGTIDE_OT_add_ai_spline,
    KINGTIDE_OT_add_starts,
    KINGTIDE_OT_scaffold_track_essentials,
)
