...
def _candidate_seed_paths() -> list[str]:
    ...
    try:
        ...
        # __file__ → .../tools/blender/kingtide_addon/multibiome_terrain.py
...
class KINGTIDE_OT_add_multibiome_terrain(Operator):
    ...
    * Skips driver empties that already exist with the expected names.
    ...
    bl_idname = "kingtide.add_multibiome_terrain"
...
def register() -> None:
    bpy.utils.register_class(KINGTIDE_OT_add_multibiome_terrain)


def unregister() -> None:
    try:
        bpy.utils.unregister_class(KINGTIDE_OT_add_multibiome_terrain)
    except RuntimeError:
        pass
