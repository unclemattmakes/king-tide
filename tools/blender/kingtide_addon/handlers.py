    ...
    if "path_wear" in pending:
        try:
            ...
            if not ok:
                ...
                # edit flow.
                print(f"[kingtide] {msg}")
        except (RuntimeError, AttributeError) as e:
            print(f"[kingtide] auto path-wear rebake errored: {e}")
...
def reload_hoverbike_libraries() -> list[str]:
    ...
    King Tide-managed library. Returns the list of basenames actually
    ...
    for lib in list(bpy.data.libraries):
        ...
        try:
            ...
            print(f"[kingtide] library reload failed for {basename}: {e}")
    return reloaded
...
def _hoverbike_load_post(*_args):
    """On every .blend open: refresh any King Tide libraries the file
    ...
    if reloaded:
        print(f"[kingtide] auto-reloaded {len(reloaded)} library file(s): {', '.join(reloaded)}")
    ...
    if upgraded_ng:
        print(
            f"[kingtide] upgraded is_modifier=True on {len(upgraded_ng)} "
    ...
    if "mat_terrain_main" in bpy.data.materials:
        try:
            ...
            print(f"[kingtide] terrain material auto-upgrade skipped: {e}")
    ...
    try:
        ...
        print(f"[kingtide] auto-reload-from-JSON skipped: {e}")
