...
Authoring loop:
  ...
  2. Click **Add Emitter** in the King Tide panel's *Emitters*
     sub-section.
...
class KINGTIDE_OT_add_emitter(Operator):
    ...
    bl_idname = "kingtide.add_emitter"
...
class KINGTIDE_PT_track_emitters(bpy.types.Panel):
    ...
    bl_label = "Emitters"
    bl_idname = "KINGTIDE_PT_track_emitters"
    ...
    def draw(self, context):
        ...
        layout.operator("kingtide.add_emitter", icon="ADD")
        layout.separator()
...
# Note: KINGTIDE_PT_track_emitters is registered by panel.py (alongside
# the other KINGTIDE_PT_track_* sub-panels) so the parent panel
# KINGTIDE_PT_panel is in bpy.types before any child tries to attach.
# Registering it here would race with panel.py's registration order,
# since `panel` is the last module in `_MODULES`.
_CLASSES: tuple[type, ...] = (KINGTIDE_OT_add_emitter,)
