"""Top-bar King Tide menu (View3D header) + viewport pie menu.

This module hangs a traditional dropdown menu off the View3D header bar
— the same surface BlenderGIS uses — so authors can find every King Tide
operator without fishing through the N-panel sidebar.

  ┌──────────────────────────────────────────────────────────────┐
  │ View   Select   Add   Object   King Tide ▾                   │  ← VIEW3D_MT_editor_menus
  └──────────────────────────────────────────────────────────────┘
...
A pie menu (``KINGTIDE_MT_pie``) is also registered and bound to
...
class KINGTIDE_MT_main(Menu):
    """Top-level King Tide dropdown shown in the View3D header."""

    bl_idname = "KINGTIDE_MT_main"
    bl_label = "King Tide"
    ...
    def _draw_track(self, context, layout) -> None:
        ...
        layout.operator("kingtide.export_track", icon="EXPORT")
        layout.operator("kingtide.lint_track", icon="CHECKMARK")
        layout.operator(
            "kingtide.reload_track_json", text="Reload from JSON", icon="FILE_REFRESH"
        )

        layout.separator()
        op = layout.operator("kingtide.open_play_url", text="Play in Browser", icon="PLAY")
        op.edit = False
        op = layout.operator(
            "kingtide.open_play_url", text="Edit in Browser", icon="GREASEPENCIL"
        )
        op.edit = True
        op = layout.operator(
            "kingtide.copy_track_url", text="Copy Play URL", icon="URL"
        )
        op.edit = False
        op = layout.operator(
            "kingtide.copy_track_url", text="Copy Edit URL", icon="URL"
        )
        op.edit = True
        layout.operator(
            "kingtide.detect_dev_servers", text="Find Dev Servers", icon="VIEWZOOM"
        )

        layout.separator()
        layout.menu("KINGTIDE_MT_add", icon="ADD")
        layout.menu("KINGTIDE_MT_build", icon="MOD_BUILD")
        layout.menu("KINGTIDE_MT_spline", icon="CURVE_NCURVE")
        layout.menu("KINGTIDE_MT_terrain", icon="RNDCURVE")
        layout.menu("KINGTIDE_MT_thumbnail", icon="RENDER_STILL")
        layout.menu("KINGTIDE_MT_utility", icon="TOOL_SETTINGS")

        layout.separator()
        layout.menu("KINGTIDE_MT_pie", text="Quick Pie (Shift+W)", icon="MESH_CIRCLE")
        layout.separator()
        layout.operator(
    ...
    def _draw_bike(self, context, layout) -> None:
        layout.operator("kingtide.export_bike", icon="EXPORT")
        layout.separator()
        op = layout.operator("kingtide.copy_bike_url", text="Copy Play URL", icon="URL")
        op.viewer = False
        op = layout.operator(
...
class KINGTIDE_MT_add(Menu):
    bl_idname = "KINGTIDE_MT_add"
    bl_label = "Add"
    ...
    def draw(self, context):
        ...
        layout.operator("kingtide.scaffold_track_essentials", icon="ADD")
        layout.operator("kingtide.add_ai_spline", icon="CURVE_NCURVE")
        layout.operator("kingtide.add_starts", icon="EMPTY_ARROWS")
        ...
        layout.operator("kingtide.add_antigrav_curve", icon="CURVE_BEZCURVE")

        layout.separator()
        layout.label(text="Gameplay")
        layout.operator("kingtide.add_ramp", text="Ramp (at cursor)", icon="ADD")
        layout.operator(
            "kingtide.add_ramp_at_spline_t",
            text="Ramp (at spline t)",
            icon="CURVE_DATA",
        )
        layout.operator(
            "kingtide.add_ramp_at_helper",
            text="Ramp (at helper)",
            icon="EMPTY_AXIS",
        )
        layout.operator(
            "kingtide.add_boost_pad", text="Boost Pad", icon="FORCE_FORCE"
        )
        layout.operator(
            "kingtide.add_boost_pad_at_helper",
            text="Boost Pad (at helper)",
            icon="EMPTY_AXIS",
        )
        layout.operator(
            "kingtide.add_antigrav_zone",
            text="Anti-Grav Zone",
            icon="ORIENTATION_GIMBAL",
        )
        layout.operator("kingtide.add_wave_zone", text="Wave Zone", icon="MOD_OCEAN")

        layout.separator()
        layout.label(text="Environment")
        layout.operator(
            "kingtide.rebuild_water_preview",
        ...
        layout.operator(
            "kingtide.add_horizon_ring", text="Horizon Ring", icon="WORLD"
        )
        layout.operator("kingtide.add_downtown", text="Downtown Block", icon="MESH_CUBE")
        layout.operator("kingtide.add_emitter", text="Emitter", icon="PARTICLES")
        layout.operator(
            "kingtide.add_scatter_zone",
            text="Scatter Zone (foliage / rocks)",
            icon="OUTLINER_OB_POINTCLOUD",
        )
        layout.operator(
            "kingtide.add_biome_palette",
            text="Biome Palette Scatter (auto-per-biome)",
            icon="OUTLINER_OB_POINTCLOUD",
        )
        layout.operator(
            "kingtide.add_scatter_stroke",
...
class KINGTIDE_MT_build(Menu):
    bl_idname = "KINGTIDE_MT_build"
    ...
    def draw(self, context):
        ...
        layout.operator("kingtide.build_road", icon="MESH_PLANE")
        layout.operator(
        ...
        layout.separator()
        layout.operator("kingtide.build_tunnel", icon="MESH_CYLINDER")
        layout.operator(
        ...
        layout.separator()
        layout.operator(
            "kingtide.build_antigrav_surface",
        ...
        layout.operator(
            "kingtide.hide_gate_preview", text="Hide Gates", icon="HIDE_ON"
        )
        layout.operator(
        ...
        layout.operator(
            "kingtide.hide_racer_preview", text="Hide Racer", icon="HIDE_ON"
        )
        layout.operator(
            "kingtide.rebuild_turn_indicators",
            text="Rebuild Turn Indicators",
            icon="TRACKING_FORWARDS",
        )
        layout.operator(
            "kingtide.hide_turn_indicators",
        ...
        layout.operator(
            "kingtide.hide_water_preview", text="Hide Water", icon="HIDE_ON"
        )
        layout.operator(
            "kingtide.rebuild_ghost_lap", text="Rebuild Ghost Lap", icon="PLAY"
        )
        layout.operator(
            "kingtide.hide_ghost_lap", text="Hide Ghost Lap", icon="HIDE_ON"
        )

        layout.separator()
        layout.label(text="Refresh placed gizmos")
        layout.operator(
            "kingtide.refresh_boost_pads",
            text="Refresh Boost Pads",
            icon="FORCE_FORCE",
        )
        layout.operator(
            "kingtide.refresh_antigrav_zones",
            text="Refresh Anti-Grav Zones",
            icon="ORIENTATION_GIMBAL",
        )
        layout.operator(
            "kingtide.refresh_wave_zones",
            text="Refresh Wave Zones",
            icon="MOD_OCEAN",
        )
        layout.operator(
            "kingtide.refresh_scatter_zones",
...
class KINGTIDE_MT_spline(Menu):
    bl_idname = "KINGTIDE_MT_spline"
    bl_label = "Spline"
    ...
    def draw(self, context):
        ...
        layout.operator(
            "kingtide.cursor_snap_to_spline",
        ...
        layout.operator(
            "kingtide.snap_spline_to_terrain",
        ...
        layout.operator(
            "kingtide.toggle_spline_antigrav",
        ...
        if context.mode == "EDIT_CURVE":
            ...
            layout.operator(
                "kingtide.set_spline_tilt_flat", text="Flat (0°)"
            )
            layout.operator(
                "kingtide.set_spline_tilt_bank_l", text="Bank L (+45°)"
            )
            layout.operator(
                "kingtide.set_spline_tilt_bank_r", text="Bank R (−45°)"
            )
            layout.operator(
                "kingtide.set_spline_tilt_wall_l", text="Wall L (+90°)"
            )
            layout.operator(
                "kingtide.set_spline_tilt_wall_r", text="Wall R (−90°)"
            )
            layout.operator(
                "kingtide.set_spline_tilt_ceiling", text="Ceiling (180°)"
            )
...
class KINGTIDE_MT_terrain(Menu):
    bl_idname = "KINGTIDE_MT_terrain"
    bl_label = "Terrain"
    ...
    def draw(self, context):
        ...
        layout.operator(
            "kingtide.import_heightmap",
            text="Import Heightmap…",
            icon="IMPORT",
        )
        layout.operator(
            "kingtide.apply_terrain_modifiers",
            text="Apply Modifiers",
            icon="MODIFIER_DATA",
        )
        layout.operator(
            "kingtide.subdivide_terrain", text="Subdivide", icon="MOD_SUBSURF"
        )
        ...
        layout.operator(
            "kingtide.enter_sculpt_mode", text="Enter Sculpt Mode", icon="BRUSH_DATA"
        )
        op = layout.operator(
            "kingtide.raise_lower_terrain", text="Raise @ cursor", icon="TRIA_UP"
        )
        op.lower = False
        op = layout.operator(
            "kingtide.raise_lower_terrain", text="Lower @ cursor", icon="TRIA_DOWN"
        )
        op.lower = True
        ...
        layout.operator(
            "kingtide.apply_terrain_vertex_colors",
        ...
        layout.operator(
            "kingtide.bake_path_worn", text="Bake Path Worn only", icon="MOD_CURVE"
        )
...
class KINGTIDE_MT_thumbnail(Menu):
    bl_idname = "KINGTIDE_MT_thumbnail"
    bl_label = "Thumbnail"
...
class KINGTIDE_MT_utility(Menu):
    bl_idname = "KINGTIDE_MT_utility"
    bl_label = "Utility"
    ...
    def draw(self, context):
        ...
        layout.operator("kingtide.lint_track", icon="CHECKMARK")
        layout.operator(
            "kingtide.refresh_track_stats",
            text="Refresh Track Stats",
            icon="FILE_REFRESH",
        )
        layout.operator(
            "kingtide.retag_scene", text="Re-tag Scene", icon="OUTLINER_DATA_FONT"
        )
        layout.separator()
        layout.operator(
            "kingtide.reload_props_library",
        ...
        if helper is not None:
            layout.operator(
                "kingtide.cursor_to_helper",
                text="Cursor → Helper",
                icon="PIVOT_CURSOR",
            )
            layout.operator(
                "kingtide.remove_placement_helper",
                text="Remove Helper",
                icon="X",
            )
...
class KINGTIDE_MT_pie(Menu):
    bl_idname = "KINGTIDE_MT_pie"
    bl_label = "King Tide Quick"
    ...
    def draw(self, context):
        ...
        pie.operator(
            "kingtide.add_ramp_at_spline_t", text="Ramp @ t", icon="ADD"
        )
        ...
        pie.operator("kingtide.export_track", text="Export Track", icon="EXPORT")
        # NW — Cursor → helper
        pie.operator(
            "kingtide.cursor_to_helper",
            text="Cursor → Helper",
        ...
        # NE — Open Play URL
        op = pie.operator("kingtide.open_play_url", text="Play", icon="PLAY")
        op.edit = False
        # SW — Build road (rebuilds road mesh from active curve)
        pie.operator("kingtide.build_road", text="Build Road", icon="MESH_PLANE")
...
def register() -> None:
    ...
    if kc is not None:
        ...
        kmi.properties.name = KINGTIDE_MT_pie.bl_idname
