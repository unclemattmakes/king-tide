...
other ``HOVERBIKE_PT_track_*`` sub-panels by ``panel.py``.
"""
...
def _resolve_repo_paths(track_id: str) -> tuple[str, str] | None:
    ...
    or ``None`` if the .blend isn't inside a hoverbike clone. Lifted
...
def render_track_hero(
    *,
    ...
    if paths is None:
        raise RuntimeError(
            f"no repo root for {blend} — save the .blend inside a hoverbike clone"
        )
...
class HOVERBIKE_OT_add_camera_hero(Operator):
    ...
    bl_idname = "hoverbike.add_camera_hero"
...
class HOVERBIKE_OT_render_track_hero(Operator):
    ...
    bl_idname = "hoverbike.render_track_hero"
...
class HOVERBIKE_OT_render_track_thumbnail(Operator):
    ...
    bl_idname = "hoverbike.render_track_thumbnail"
    ...
    def execute(self, context):
        ...
        paths = _resolve_repo_paths(track_id)
        if paths is None:
            self.report({"ERROR"}, "no repo root — save inside a hoverbike clone")
            return {"CANCELLED"}
        _, tile_path = paths
...
_CLASSES: tuple[type, ...] = (
    HOVERBIKE_OT_add_camera_hero,
    HOVERBIKE_OT_render_track_hero,
    HOVERBIKE_OT_render_track_thumbnail,
)
