...
def _repo_root_from_blend() -> str | None:
    ...
    sibling (same heuristic as ``find_repo_root`` in _legacy)."""
...
class HOVERBIKE_OT_add_scatter_zone(Operator):
    ...
    bl_idname = "hoverbike.add_scatter_zone"
    ...
    def execute(self, context):
        ...
        if result is None:
            self.report(
                ...
                "Run `pnpm seed:props` and re-save this .blend inside a hoverbike repo clone.",
            )
            return {"CANCELLED"}
        empty, surf = result
...
class HOVERBIKE_OT_refresh_scatter_zones(Operator):
    ...
    bl_idname = "hoverbike.refresh_scatter_zones"
...
_CLASSES: tuple[type, ...] = (
    HOVERBIKE_OT_add_scatter_zone,
    HOVERBIKE_OT_refresh_scatter_zones,
)
