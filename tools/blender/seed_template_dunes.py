...
3. When the silhouette reads right, **Apply the modifier**, edit the AI
   spline / gates / starts on top, then export via the King Tide addon's
...
def build_template_dunes_group() -> bpy.types.NodeTree:
    ...
    # ── Additive clamp + switch ─────────────────────────────────────
    # See seed_template_island.py. Additive=True clamps Z to max(0, z)
    # so the sub-group only raises the input geometry — useful when
    # stacked in HV_TemplateTerrain. Additive=False preserves the
    # signed Z so the oasis basin can dip below z=0 as authored.
    n_add_clamp = _add_node(g, "ShaderNodeMath", 550, -100, operation="MAXIMUM")
    n_add_clamp.name = "HV_Additive_Clamp"
    n_add_clamp.label = "HV_Additive_Clamp"
    n_add_clamp.inputs[1].default_value = 0.0
    g.links.new(n_sum4.outputs[0], n_add_clamp.inputs[0])
    n_add_sw = _add_node(g, "GeometryNodeSwitch", 650, -200, input_type="FLOAT")
    n_add_sw.name = "HV_Additive_Switch"
    n_add_sw.label = "HV_Additive_Switch"
    g.links.new(p_in.outputs["Additive"], n_add_sw.inputs[0])
    g.links.new(n_sum4.outputs[0],        n_add_sw.inputs[1])
...
def _load_addon_module():
    ...
    if not os.path.exists(init_file):
        print(f"[seed-template-dunes] WARNING: {init_file} not found; skipping previews")
        return None
    ...
    if spec is None or spec.loader is None:
        print(f"[seed-template-dunes] WARNING: could not load spec for {init_file}; skipping previews")
        return None
