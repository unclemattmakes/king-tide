...
export interface TuckHud {
   ...
   */
  update(leanAmount: number, factor: number, capBonusPct: number, active: boolean): void
...
export function createTuckHud(sweetSpot: number): TuckHud {
  ...
  `
  // Notch sits at the sweet-spot lean fraction — set once.
  ...
  function classify(leanAmount: number, factor: number, active: boolean): TuckState {
    ...
    return leanAmount > sweetSpot ? 'over' : 'build'
  }

  return {
    update(leanAmount, factor, capBonusPct, active) {
      ...
      const state = classify(lean, factor, active)
