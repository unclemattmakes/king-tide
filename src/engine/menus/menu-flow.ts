...
export function runMenuFlow(opts: MenuFlowOpts): Promise<MenuFlowResult> {
  ...
  function buildV1TrackCard(t: V1TrackEntry): HTMLElement {
    ...
    // to an accent-tinted placeholder block: the tint is this inline
    // `background-image` longhand (NOT the `background` shorthand, whose
    // implicit background-color reset would defeat `.thumb-pending`'s base).
    const thumbHtml = disabled
      ? `<div class="thumb thumb-pending" style="background-image: linear-gradient(135deg, ${t.accent}26, transparent 70%)"></div>`
        ...
        <div class="bc-credits">
          <section class="bc-credit-group">
            ...
            <p>${GAME_TITLE_PROSE}&rsquo;s hand-painted surfaces build on the
              ...
              scanned brush maps were sliced, recentred and baked into the tiling
              brush-stroke texture behind the props&rsquo;, buildings&rsquo; and
              terrain&rsquo;s painted surfaces.</p>
