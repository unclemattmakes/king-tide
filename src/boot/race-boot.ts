 ...
 * race — so the cold-boot menu path never evaluates this module graph
 * (three.js + every render/sim system), which was a multi-second
...
export async function bootRace(appEl: HTMLElement) {
  ...
  // Phase 2 — core subsystems. Renderer (WebGPU adapter/device) and the
  // physics world (Rapier WASM fetch + instantiate) depend on nothing but
  // the page, and not on each other — init them concurrently.
  setLoadingMessage('Starting engine…')
  const [rendererBundle, phys] = await Promise.all([createRenderer(appEl), createPhysicsWorld()])
  ...
  // Early fetch kicks — everything whose URL is already known from the
  // params starts downloading NOW, overlapped with the track/water/sky work
  // below instead of serialized after it. Every loader memoizes by URL, so
  // the existing consumption points await these exact promises. The `void
  // p.catch(...)` forks only mark early rejections as handled (no
  // unhandledrejection noise during the gap); the consuming await still
  // observes the original failure exactly as before.
  const riderModeParam = params.get('rider')
  const bikeIdsToLoad = [
    ...new Set([playerVariant.id, 'racer', ...(timeTrialMode ? [] : Object.keys(BIKE_VARIANTS))]),
  ]
  const bikeGlbPromises = bikeIdsToLoad.map((id) => loadBike(assetUrl(`/assets/bikes/${id}.glb`)))
  for (const p of bikeGlbPromises) void p.catch(() => {})
  const gatePropPromise = loadGateProp()
  void gatePropPromise.catch(() => {})
  const riderRigPromise =
    riderModeParam === 'capsule' || riderModeParam === 'capsules'
      ? null
      : loadProp(assetUrl('/assets/props/cc0/rider_mannequin.glb'))
  if (riderRigPromise) void riderRigPromise.catch(() => {})
  // Warm the HTTP cache for the track JSON + its environment GLB — the
  // single biggest download (8–20 MB on dressed tracks). On deployed builds
  // that fetch otherwise starts only when `loadTrackForBoot` reaches it,
  // serialized after renderer + physics init; prefetching here overlaps the
  // network time with all of that. Purely additive: the loader's own
  // fetches hit the warmed cache (same URLs), and a procedural/GLB-direct
  // track just 404s the JSON probe harmlessly.
  void (async () => {
    try {
      const probe = await fetch(`/tracks/${encodeURIComponent(trackId)}.json`)
      if (!probe.ok) return
      const json = (await probe.json()) as { environmentGlb?: string }
      if (json.environmentGlb) await fetch(assetUrl(json.environmentGlb))
    } catch {
  ...
  // Pre-race cinematic intro — decided EARLY (inputs: room/skipintro/bench
  // params + player prefs, all parsed above) because the scenery-warm strategy
  // right below keys on it. Multiplayer skips the cinematic (players joining a
  // room want the grid, and a fly-by would fight the lobby UX); replay playback
  // boots into the saved race flow; `?skipintro=1` forces it off (QA + e2e);
  // bench mode implies skip-intro (the cinematic is camera cuts, not race
  // load, and would dominate the bench warmup+measure window).
  ...
  // Hide the env GLB's vinyl scenery NOW — before any pipeline warm can see
  // it — so the streamed warm below compiles only the player-essential set
  // and the scenery streams in post-ready (progressive-warm.ts). ONLY for
  // no-intro modes: when the cinematic plays, the loader is held for a fully
  // dressed scene anyway, and compiling the (material-deduped) scenery inside
  // the eager boot pre-warm render is far cheaper than the deferred path's
  // per-group `createRenderPipelineAsync` waits — the FIRST of those stalls
  // ~8–12 s behind the essential set's driver-side pipeline backlog regardless
  // of group count (measured 2026-07-01, iGPU dev box: mexico-city intro boot
  // 20.1 s deferred vs 8.0 s undeferred, sandbar 18.1 s deferred, with
  // comparable-or-better cinematic frame health undeferred). With ?progwarm=0
  // the scenery always stays visible (same whole-scene warm, any mode).
  const deferScenery = !editMode && params.get('progwarm') !== '0' && introMode === 'off'
  const envScenery = deferScenery ? collectVinylScenery([environmentGlbRoot]) : []
  for (const m of envScenery) m.visible = false
  ...
  // Early pipeline warm (docs/boot-overhaul-plan.md): one REAL eager render
  // of what's already in the scene — terrain + env GLB (scenery hidden
  // above), the water material (the single biggest pipeline), sky dome,
  // horizon. An eager `pipeline.render()` is the one warm that's correct by
  // construction for the water's scene-depth/MSAA coupling (the async
  // subtree path keys those pipelines wrong — see post-pipeline.ts) — and
  // it leaves the PassNode state real, which is the precondition for the
  // async compiles the progressive scenery warm runs post-ready. Splitting
  // the warm here halves the final pre-warm stall and lets the loading
  // screen repaint between the two.
  setLoadingMessage('Warming up water + terrain…')
  try {
    const earlyPipeline = getActivePostPipeline()
    const rEarly = renderer as unknown as {
      compileAsync?: (scene: unknown, camera: unknown) => Promise<void>
    }
    const tEarly = performance.now()
    if (earlyPipeline) {
      await earlyPipeline.compileAsync()
    } else if (typeof rEarly.compileAsync === 'function') {
      await rEarly.compileAsync(scene, camera)
    }
    bootStat('warmEarly', Math.round(performance.now() - tEarly))
  } catch (err) {
    // Best-effort — the final pre-warm below covers the same ground.
    // eslint-disable-next-line no-console
    console.warn('[main] early pipeline warm failed:', err)
  }
  ...
  // the asset isn't available. (Fetch was kicked at the top of boot.)
  const gatePropTemplate = await gatePropPromise
  ...
  // Defer the editor-authored prop scenery exactly like the env scenery
  // above — the essential remainder (gates, wave-rider/animated props)
  // compiles in the final pre-warm render below.
  const propScenery = deferScenery && propsGroup ? collectVinylScenery([propsGroup]) : []
  for (const m of propScenery) m.visible = false
  ...
  // Fetches were kicked at the top of boot (right after the variant
  // resolved) — by now they've been downloading underneath the track/water/
  // sky work, so this await is usually just the parse remainder.
  const loadedBikeGlbs = await Promise.all(bikeGlbPromises)
  ...
  // (`introMode` itself is decided early, before the env GLB loads — the
  // scenery-warm deferral keys on it. See the block above `deferScenery`.)
  ...
  if (!riderRigPromise) {
    ...
    try {
      // Kicked at the top of boot alongside the bike GLBs.
      riderRig = await riderRigPromise
    } catch (err) {
  ...
  try {
    ...
    // render state before the warm walks it below. It's preloaded +
    ...
    if (deferScenery) {
      // Scenery meshes were hidden the moment their roots loaded (so
      // neither warm render compiled them); this wraps them in the
      // post-ready reveal handle.
      progWarm = deferSceneryWarm([...envScenery, ...propScenery])
    ...
    // The early warm at sky-time already compiled the heavyweights (water,
    // terrain, sky), so this render only pays the late arrivals — bike
    // fields, riders, gates, pickups, fx.
    const activePipeline = getActivePostPipeline()
    const tFinalWarm = performance.now()
    if (activePipeline) {
      await activePipeline.compileAsync()
    } else if (typeof r.compileAsync === 'function') {
      await r.compileAsync(scene, camera)
    }
    bootStat('warmFinal', Math.round(performance.now() - tFinalWarm))
  } catch (err) {
    ...
    progWarm.reveal({
      compile: warmPipeline
        ...
        : typeof warmRenderer.compileAsync === 'function'
          ...
          : undefined,
      // Scenery is only deferred on no-intro modes (deferScenery keys on
      // introMode), so this reveal always streams in DURING racing — default
      // concurrency 1, one pipeline-group per frame, keeps each compile off
      // the hot frame. The intro path doesn't defer at all: its scenery
  ...
  // No loader hold is needed for the cinematic: the intro path never defers
  // scenery (see `deferScenery`), so by this point the eager pre-warm above
  // has already compiled the fully-dressed scene the fly-through will sweep.
